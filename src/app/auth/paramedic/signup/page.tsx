...
export default function ParamedicAuth() {
  ...
  const [regData, setRegData] = useState({ name: '', phone: '', vehicle: '' });
    ...
    <div className="min-h-screen bg-secondary flex flex-col">
      ...
      <main className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-md">
          <div className="mb-8">
            <span className="text-[10px] font-mono uppercase tracking-widest text-primary/60">Portal 02</span>
            <h1 className="text-3xl font-bold text-ink mt-1">Paramedic access</h1>
            <p className="text-dark mt-1">Login or register to start accepting dispatches</p>
          </div>

          <div className="flex mb-6 border border-primary/20 rounded-lg overflow-hidden bg-paper">
            <button
              onClick={() => { setTab('login'); setError(''); }}
              className={`flex-1 py-2.5 text-sm font-semibold transition-colors ${tab === 'login' ? 'bg-ink text-white' : 'text-dark/60 hover:text-dark'}`}
            >Login</button>
            <button
              onClick={() => { setTab('register'); setError(''); }}
              className={`flex-1 py-2.5 text-sm font-semibold transition-colors ${tab === 'register' ? 'bg-ink text-white' : 'text-dark/60 hover:text-dark'}`}
            >Register</button>
          </div>
          ...
          {tab === 'login' ? (
            <form onSubmit={handleLogin} className="space-y-4">
              <div>
                <label className="block text-xs font-mono uppercase tracking-widest text-dark/60 mb-1.5">Registered Phone</label>
                <input
              ...
              <p className="text-center text-sm text-dark/60">
                New here?{' '}
                <button type="button" onClick={() => setTab('register')} className="text-primary font-semibold underline underline-offset-2">Register</button>
              </p>
            </form>
          ) : (
            <form onSubmit={handleRegister} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Rahul Deshmukh" required value={regData.name}
              ...
              <div>
                ...
                <input
                  className="w-full px-4 py-3 bg-paper border border-primary/20 rounded-lg text-ink placeholder-dark/40 focus:outline-none focus:border-primary transition-colors font-mono"
                  placeholder="+91 98765 43210" type="tel" required value={regData.phone}
                  onChange={(e) => setRegData({ ...regData, phone: e.target.value })}
                />
              </div>
              <div>
                <label className="block text-xs font-mono uppercase tracking-widest text-dark/60 mb-1.5">Vehicle Reg. No.</label>
                <input
                  className="w-full px-4 py-3 bg-paper border border-primary/20 rounded-lg text-ink placeholder-dark/40 focus:outline-none focus:border-primary transition-colors font-mono"
                  placeholder="MH-12-XX-1234" required value={regData.vehicle}
              ...
              <p className="text-center text-sm text-dark/60">
                Already registered?{' '}
                <button type="button" onClick={() => setTab('login')} className="text-primary font-semibold underline underline-offset-2">Login</button>
              </p>
            </form>
          )}
        </div>
